...
def get2017CSO() -> pd.DataFrame:
    ...
    risk.extend((['nonsmoker super_preferred', 'nonsmoker preferred', 'nonsmoker residual']*2 + ['smoker preferred', 'smoker residual']*2)*2)
    ...
    pGrouping, pSmoke, pBasis, pGender = list(zip(*product(['loaded smoker_distinct gender_blended'], ['nonsmoker', 'smoker'], ['ANB', 'ALB'], [f'{percent}%_male' for percent in [20, 40, 50, 60, 80]])))
...
def get2015VBT() -> pd.DataFrame:
    ...
    pGender, pBasis, pRisk =zip(*product(['female'], ['ALB', 'ANB'], [f"nonsmoker RR{x}" for x in [50, 60, 70, 80, 90, 100, 110, 125, 150, 175]]))
    gender.extend(pGender)
    ...
    pGender, pBasis, pRisk =zip(*product(['female'], ['ALB', 'ANB'], [f"smoker RR{x}" for x in [75, 100, 125, 150]]))
    gender.extend(pGender)
    ...
    pGender, pBasis, pRisk =zip(*product(['male'], ['ALB', 'ANB'], [f"nonsmoker RR{x}" for x in [50, 60, 70, 80, 90, 100, 110, 125, 150, 175]]))
    gender.extend(pGender)
    grouping.extend(['relative_risk']*20)
    risk.extend(pRisk)
    age_basis.extend(pBasis)
    ...
    pGender, pBasis, pRisk =zip(*product(['male'], ['ALB', 'ANB'], [f"smoker RR{x}" for x in [75, 100, 125, 150]]))
    gender.extend(pGender)
    grouping.extend(['relative_risk']*8)
    risk.extend(pRisk)
    age_basis.extend(pBasis)
