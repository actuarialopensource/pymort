...
def test_PyXML():
    ...
    assert (
        contentClassification.ProviderName
        == "American Academy of Actuaries along with the Society of Actuaries"
    )
    assert (
        contentClassification.TableReference
    ...
    assert (
        contentClassification.TableName
        == "2017 Loaded CSO Composite Gender-Blended 50% Male ANB"
    )
    assert (
        contentClassification.TableDescription
        == "2017 Loaded CSO Composite Gender-Blended, Male, 50%, Age Nearest Birthday. Minimum Age: 0. Maximum Age: 95."
    )
    assert (
        contentClassification.Comments
    ...
    assert contentClassification.KeyWords == [
        "Select",
        "CSO/CET",
    ...
    assert meta0.ScalingFactor == 0.0
    assert meta0.DataType == "Floating Point"
    assert meta0.Nation == "United States of America"
    assert (
        meta0.TableDescription
        == "2017 Loaded CSO Composite Gender-Blended, Male, 50%, Select and Ultimate Table. Basis: Age Nearest Birthday. Minimum Select Age: 0. Maximum Select Age: 95."
    )
    ...
    assert 0.00029 == list(values0.loc[0, 1])[0]
    assert 0.00072 == list(values0.loc[5, 23])[0]

    # Tests for the ultimate table
    meta1 = xml.Tables[1].MetaData
    assert meta1.ScalingFactor == 0.0
    assert meta1.DataType == "Floating Point"
    assert meta1.Nation == "United States of America"
    assert (
        meta1.TableDescription
        == "2017 Loaded CSO Composite Gender-Blended, Male, 50%, Select and Ultimate Table. Basis: Age Nearest Birthday. Minimum Ultimate Age: 0. Maximum Ultimate Age: 120"
    )
    ...
    assert 0.00029 == values1.loc[0]["vals"]
    assert 0.00248 == values1.loc[50]["vals"]
